...
## One-off diagnostic for GlacierFaultCourse: builds it, spawns a full field of
...
##   godot --headless --path . --script tools/probe_glacier_fault.gd \
##     --fixed-fps 60 --disable-render-loop --quit-after 3600

const COURSE: GDScript = preload("res://scripts/course/glacier_fault_course.gd")
...
## construction time, and `GlacierFaultCourse.build()` starts a `Timer` (the
## icicle spawner) that needs `is_inside_tree()` true the moment it starts.
