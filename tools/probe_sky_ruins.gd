...
## One-off diagnostic for SkyRuinsCourse: builds it, spawns a full field of
...
##   godot --headless --path . --script tools/probe_sky_ruins.gd \
##     --fixed-fps 60 --disable-render-loop --quit-after 3600

const COURSE: GDScript = preload("res://scripts/course/sky_ruins_course.gd")
...
## construction time, and `SkyRuinsCourse.build()` starts a `Timer` (the
